...
namespace AviFile
{
	...
	public class VideoStream : AviStream
	{
        ...
        private static readonly int SIZE_BIH = Marshal.SizeOf(typeof(Avi.BITMAPINFOHEADER));
		...
		public PixelFormat ConvertBitCountToPixelFormat(int bitCount){
			String formatName;
			if(bitCount > 16){
        ...
        public IntPtr GetRawBitmap(int position, out Avi.BITMAPINFOHEADER bih, out int dataLength)
        {
            ...
            if (getFrameObject == 0)
            {
                throw new Exception("GetFrameOpen has not been called yet");
            }

            int dib = Avi.AVIStreamGetFrame(getFrameObject, firstFrame + position);
            bih = (Avi.BITMAPINFOHEADER)Marshal.PtrToStructure(new IntPtr(dib), typeof(Avi.BITMAPINFOHEADER));

			if(bih.biSizeImage < 1){
				throw new Exception("Exception in VideoStreamGetFrame");
			}

            dataLength = bih.biSizeImage;
            if (bih.biBitCount < 16)
            {
                dataLength += Avi.PALETTE_SIZE;
            }
            return new IntPtr(dib + SIZE_BIH);
        }
        ...
        public int GetNextKeyFrame(int pos)
        {
            return Avi.AVIStreamFindSample(StreamPointer, pos + 1, Avi.FIND_NEXT | Avi.FIND_KEY);
        }
        public int GetPrevKeyFrame(int pos)
        {
            return Avi.AVIStreamFindSample(StreamPointer, pos - 1, Avi.FIND_PREV | Avi.FIND_KEY);
        }
        public int GetNextNonEmptyFrame(int pos)
        {
            return Avi.AVIStreamFindSample(StreamPointer, pos + 1, Avi.FIND_NEXT | Avi.FIND_ANY);
        }
        public int GetPrevNonEmptyFrame(int pos)
        {
            return Avi.AVIStreamFindSample(StreamPointer, pos - 1, Avi.FIND_PREV | Avi.FIND_ANY);
        }
        public FrameType GetFrameType(int pos) {
            if (Avi.AVIStreamFindSample(StreamPointer, pos, Avi.FIND_PREV | Avi.FIND_KEY) == pos)
            {
            ...
            {
                int frameSize=0;
                if (Avi.AVIStreamRead(StreamPointer, pos, 1, IntPtr.Zero, 0, ref frameSize, 0) != 0)
                {
                    throw new Exception("Exception in AVIStreamRead");
                }
                if (frameSize == 0)
                {
